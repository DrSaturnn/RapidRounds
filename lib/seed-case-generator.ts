...
function generateCasesFromSchemaNodeUncached(
  ...
  return selectedVariants.map((variant) => {
    ...
    return {
      ...
      canonicalAnswer: node.canonicalAnswer,
      displayAnswer: node.displayAnswer,
      literalAnswer: node.literalAnswer,
      abstractConcept: node.abstractConcept,
      conceptType: node.conceptType,
      aliases: node.aliases,
      acceptableAnswerPatterns: node.acceptableAnswerPatterns,
      unacceptableNearMisses: node.unacceptableNearMisses,
      explanation,
      clueMap,
      reasoningObject,
      conceptCard: reasoningObject.conceptCard,
