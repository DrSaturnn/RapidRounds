...
describe("Aster companion for foundational Rapid Round", () => {
  ...
  test("keeps Aster governed by canonical design-system assets and rules", () => {
    ...
    const canonicalConstitution = readFileSync("design-system/aster/canonical/ASTER_CONSTITUTION.md", "utf8");
    const proportions = readFileSync("design-system/aster/canonical/proportions.md", "utf8");
    const manifest = readFileSync("design-system/aster/runtime/asset_manifest.json", "utf8");
    const runtimeAsset = readFileSync("public/aster/runtime/neutral.png");
    const source = readFileSync("components/PracticePanel.tsx", "utf8");

    assert.ok(runtimeAsset.byteLength > 100_000);
    ...
    assert.match(canonicalConstitution, /single rigged Blender model/i);
    assert.match(proportions, /Orthographic Requirement/i);
    assert.match(manifest, /single rigged Blender model/);
    assert.match(manifest, /CSS robot reconstruction/);
    assert.match(doNotDeviate, /approved runtime derivatives/i);
    assert.match(doNotDeviate, /Do Not Deviate/i);
    assert.match(runtimeReadme, /Do not create independent Aster implementations/i);
    assert.match(runtimeReadme, /runtime\/neutral\.png/);
