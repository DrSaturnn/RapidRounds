...
describe("Aster companion for foundational Rapid Round", () => {
  ...
  test("uses a reusable Aster avatar with mood, size, and shadow variants", () => {
    ...
    assert.match(source, /getAsterProductionAsset/);
    assert.match(source, /rr-aster-production-image/);
    assert.match(assets, /ASTER_V1_NEUTRAL_ASSET/);
    assert.match(assets, /\/aster\/production\/neutral\.png/);
    ...
    assert.match(css, /\.rr-aster-production-image/);
  ...
  test("keeps Aster governed by canonical design-system assets and rules", () => {
    ...
    const manifest = readFileSync("design-system/aster/production/asset_manifest.json", "utf8");
    const pipeline = readFileSync("design-system/aster/production/PIPELINE.md", "utf8");
    const productionAsset = readFileSync("public/aster/production/neutral.png");
    const source = readFileSync("components/PracticePanel.tsx", "utf8");

    assert.ok(productionAsset.byteLength > 100_000);
    assert.match(constitution, /Aster v1\.0/i);
    assert.match(constitution, /glossy black visor/i);
    assert.match(constitution, /chest crystal/i);
    assert.match(manifest, /singleSourceOfTruth/);
    assert.match(manifest, /documentationSheetCroppingAllowed": false/);
    assert.match(pipeline, /Do not crop runtime assets from documentation sheets/i);
    ...
    assert.match(runtimeReadme, /production\/neutral\.png/);
